                    ...
                    else
                    {
                        ...
                        foreach (var stream in filesReadToProvider.Contents)
                        {
                            ...
                            if (fRequest.Length > 2)
                            {
                                ...
                                TraceUtility.WriteTrace("AttachmentController", "UploadStore:Written:" + strFilePath);
                                ProjectFieldProcessor processor = new ProjectFieldProcessor();
                                string strStoreNumbers;
                                Dictionary<string, IProjectExcelFields> objDictionary = processor.ImportFields(strFilePath, nProjectType, out strStoreNumbers);
                                fields = processor.UpdateStoreExistFields(objDictionary, (int) nProjectType, strStoreNumbers, nBrandId, nRolloutId);
                        ...
                        return new HttpResponseMessage(HttpStatusCode.OK)
                        {
                            Content = new ObjectContent<List<IProjectExcelFields>>(fields, new JsonMediaTypeFormatter())
                        };
                    }
                }
                catch (System.Exception ex)
                            ...
                            do
                            {
                                ...
                                while (reader.Read())
                                {
                                    ...
                                    if (projectType != "" && storeNumber != "")
                                    {
                                        ...
                                        objProjectExcel.getModelFromColumns(reader, dtNew);
