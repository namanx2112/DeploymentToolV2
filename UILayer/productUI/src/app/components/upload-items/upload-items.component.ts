...
export class UploadItemsComponent {
  ...
  constructor(private service: ExStoreService, @Inject(MAT_DIALOG_DATA) public data: any) {
    if (typeof data != undefined) {
      ...
      this.afterImport = data.afterImport;
      this.nInstanceId = data.nInstanceId;
    }
  ...
  uploadFileToActivity() {
    if (this.fileToUpload != null) {
      if (this.fileToUpload.name.toLowerCase().endsWith("xlsx")) {
        this.service.ImportItems("VendorParts", this.fileToUpload, this._curBrandId, this.nInstanceId).subscribe((data: any[]) => {
  ...
  UploadItems() {
    this._curTab.my_service.Import(this.selectedItems).subscribe((x: any[]) => {
      this.afterImport(x);
    });
  }
